...
As a **fork-ts-checker-webpack-plugin** user, you are the perfect candidate to help us improve our documentation: typo corrections, clarifications, more examples, etc. Take a look at the [documentation issues that need help](https://github.com/pkg-nec/fork-ts-checker-webpack-plugin/labels/docs).
...
Issues with the [design label](https://github.com/pkg-nec/fork-ts-checker-webpack-plugin/labels/design) are meant to discuss the implementation of new features. Participating in the discussion is a good opportunity to get involved and influence the future direction of **fork-ts-checker-webpack-plugin**.

### Fix bugs and implement features

Confirmed bugs and ready-to-implement features are marked with the [help wanted label](https://github.com/pkg-nec/fork-ts-checker-webpack-plugin/labels/bug). Post a comment on an issue to indicate you would like to work on it and to request help from the contributors and the community.
...
$ git remote add upstream https://github.com/pkg-nec/fork-ts-checker-webpack-plugin
...
The first public pkg-nec release is a manual publication of the prepared, tested tarball. Follow the [release guide](docs/RELEASING.md): it packs the scoped `@pkg-nec/fork-ts-checker-webpack-plugin` package once, validates that archive, and publishes that same archive on [npm](https://www.npmjs.com/package/@pkg-nec%2Ffork-ts-checker-webpack-plugin). npm provenance is intentionally deferred to a later release workflow.

The repository retains its existing [`📦🚀 semantic-release`](https://github.com/semantic-release/semantic-release) configuration for future automation; contributors should not change that configuration as part of this manual-first release.
...
up to date with the `main` and then [open a Pull Request](https://github.com/pkg-nec/fork-ts-checker-webpack-plugin/compare/main...).
